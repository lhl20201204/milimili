...
Enemy.prototype.getNowDirectionIndex = function () {
...
Enemy.prototype.monitorJump = function (map) {
  const heightLevel = map.findHigherLevel(map.tranX + this.x, this.heightInMap, map.map)
...
Enemy.prototype.drop = function (map, fn) {
  ...
  const d = (nextLevel - this.y) / timerCount
  let count = 0
  this.timer = setInterval(
    function () {
      count++
...
Enemy.prototype.resetAttribute = function () {
  ...
  this.facingRight = faceR.indexOf(this.kind) !== -1
  this.state = 0
  this.heightInMap = this.y
