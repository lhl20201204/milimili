...
export function shot (global, deg, enemy) {
  ...
  const w = constValue.bulletArrays[player.bulletKind][0]
  const h = constValue.bulletArrays[player.bulletKind][1]
  ...
  const x = player.x + (player.getWidth() / 8) * width.get(index)
  const y = player.y + (player.getHeight() / 8) * height.get(index)
  const tranX = constValue.bulletArrays[player.bulletKind][2]
  const tranY = constValue.bulletArrays[player.bulletKind][3]
  if (player.bulletKind !== 3) {
    const bullet = reactive(new Bullet(player.bulletKind, deg, x, y, w, h, tranX, tranY))
    if (!enemy) {
    ...
    for (let i = 0; i < 5; i++) {
      const bullet = reactive(new Bullet(player.bulletKind, deg - 30 + 15 * i, x, y, w, h, tranX, tranY))
      if (!enemy) {
        global.playerBullets.push(bullet)
        bullet.timerStart(global, bullet)
      } else {
